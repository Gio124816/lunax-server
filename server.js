...
const instagramDirectRoutes = require('./routes/instagram-direct');
const accountsRoutes = require('./routes/accounts');
const folderAliasesRoutes = require('./routes/folder-aliases');
...
// — META WEBHOOK — same reasoning: signature verification (routes/webhook.js)
// needs the exact raw bytes Meta signed, not the already-parsed JSON the
// global body parser below would otherwise produce.
...
// — RATE LIMITING ———————————————————
// globalLimit applies to every route, including ordinary reads (GET
// /accounts, /posts, /comments, Analytics, Ad Campaigns, etc.) — 200/15min
// was tripping during completely normal active use (several screens each
// fetching on load, reconnecting a few accounts while testing multi-
// account) well before anything resembling actual abuse. Raised to a much
// higher ceiling that still catches real abuse patterns without punishing
// a real session doing real work. authLimit and aiLimit stay as strict as
// they were — those protect specifically against login brute-forcing and
// runaway AI costs, a different concern than general API traffic.
...
// No requireAuth — Meta's own servers call this directly, not a logged-in
// user; routes/webhook.js verifies the request is genuinely from Meta via
// the X-Hub-Signature-256 check instead.
...
app.use('/oauth', instagramDirectRoutes);

app.use('/comments', requireAuth, commentsRoutes);
app.use('/analytics', requireAuth, analyticsRoutes);
// Applies requireAuth per-route internally (same pattern as team.js below),
// not wrapped here — this was the missing piece: accounts.js existed and
// SettingsView.swift's fetchAccounts() already expected this exact route,
// but nothing in server.js ever required or mounted it, so every call
// silently 404'd into the catch-all handler instead of erroring loudly.
app.use('/accounts', accountsRoutes);
// Same per-route-auth pattern as accounts.js directly above.
app.use('/folder-aliases', folderAliasesRoutes);
