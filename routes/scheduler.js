...
async function publishToTarget(post, account) {
  ...
  if (account.platform === 'instagram') {
    ...
    return { platformPostId: pub.id || null };
  }
    ...
    for (const post of due) {
      ...
      if (targets.length > 0) {
        ...
        for (const target of targets) {
          try {
            const { platformPostId } = await publishToTarget(post, target);
            db.prepare(`UPDATE post_targets SET status = 'posted', platform_post_id = ?, posted_at = ?, error_message = NULL WHERE id = ?`)
              .run(platformPostId, now, target.target_id);
            const label = `${target.platform}${target.account_name ? ' (' + target.account_name + ')' : ''}`;
            results.push(label);
            console.log(`[Scheduler] Multi-target publish success — ${label} for post ${post.id}`);
          } catch (e) {
