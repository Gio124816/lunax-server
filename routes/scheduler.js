    ...
    try {
      ...
      if (canDoIgViaDirect) {
        ...
        if (post.media_url && post.media_type === 'video') {
          console.log(`[Scheduler] (IG-direct) Uploading video for post ${post.id}...`);
        ...
        if (creationId) {
          ...
          console.log(`[Scheduler] Instagram (direct) publish success — id=${publishedId} for post ${post.id}`);
        }
      } else {
