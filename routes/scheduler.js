...
// ── MULTI-TARGET PUBLISH (mass-posting to explicit accounts) ──────────────
// Covers Facebook Page + Instagram feed/Stories, TikTok, YouTube, and
// LinkedIn — all reusing their existing single-account upload logic via a
// synthetic "user" object (see below), rather than duplicating each
// platform's upload/polling code a second time. Facebook Stories aren't
// covered — Meta's Stories publishing for Pages works quite differently
// from the standard Graph API feed/photo/video endpoints used here, so
// it's left out rather than guessed at. Also still uses the simpler
// file_url video endpoint for Facebook rather than the full resumable-
// upload dance publishPost() uses, since that needs a separate user-level
// token alongside the page token that multi-account connections (possibly
// a different Meta login entirely) can't guarantee exists.
async function publishToTarget(post, account) {
  const hashtags = JSON.parse(post.hashtags || '[]');
  const caption = post.caption + (hashtags.length ? '\n\n' + hashtags.map(h => '#' + h).join(' ') : '');
  ...
  if (account.platform === 'facebook') {
    ...
    if (post.media_url && post.media_type === 'video') {
      const r = await metaPost(`/${pageId}/videos`, token, { file_url: post.media_url, description: caption });
      return { platformPostId: r.post_id || r.id || null };
    } else if (post.media_url && post.media_type === 'image') {
      const r = await metaPost(`/${pageId}/photos`, token, { url: post.media_url, caption });
      return { platformPostId: r.post_id || r.id || null };
    } else {
      const r = await metaPost(`/${pageId}/feed`, token, { message: caption });
      return { platformPostId: r.id || null };
    }
  }

  if (account.platform === 'instagram') {
    const igId = account.platform_account_id;
    // also_share_to_story only applies to feed posts, not Stories themselves
    // — same rule as the single-account path above.
    const alsoShareToStory = !isStory && !!post.also_share_to_story;
    let creationId;
    if (post.media_url && post.media_type === 'video') {
      // Stories don't take a caption via the API — Meta rejects it, so it's
      // only included for the feed/Reels path.
      const params = isStory
        ? { media_type: 'STORIES', video_url: post.media_url }
        : { media_type: 'REELS', video_url: post.media_url, caption };
      const r = await metaPost(`/${igId}/media`, token, params);
      creationId = r.id;
      let ready = false;
      for (let i = 0; i < 40; i++) {
      ...
      const params = isStory
        ? { media_type: 'STORIES', image_url: post.media_url }
        : { image_url: post.media_url, caption };
      const r = await metaPost(`/${igId}/media`, token, params);
      creationId = r.id;
    } else {
      throw new Error('Instagram requires media');
    }
    const pub = await metaPost(`/${igId}/media_publish`, token, { creation_id: creationId });
    const publishedId = pub.id || null;

    // ─── Also share to Story ───────────────────────────────────────────
    // Same soft-failure treatment as the single-account path: the main
    // feed/Reel post already succeeded, so a Story-share problem shouldn't
    // fail the whole target — just note it and move on.
    let storyNotice = null;
    if (alsoShareToStory && publishedId && post.media_url) {
      try {
        const storyParams = { media_type: 'STORIES' };
        if (post.media_type === 'video') storyParams.video_url = post.media_url;
        else storyParams.image_url = post.media_url;
        const storyR = await metaPost(`/${igId}/media`, token, storyParams);
        const storyContainerId = storyR.id;

        if (post.media_type === 'video') {
          let storyReady = false;
          for (let i = 0; i < 40; i++) {
            await new Promise(res => setTimeout(res, 5000));
            const s = await fetch(`${API_BASE}/${storyContainerId}?fields=status_code&access_token=${token}`).then(r => r.json());
            if (s.status_code === 'FINISHED') { storyReady = true; break; }
            if (s.status_code === 'ERROR') throw new Error('Story processing failed on Meta');
          }
          if (!storyReady) throw new Error('Story processing timed out after 200s');
        }

        await metaPost(`/${igId}/media_publish`, token, { creation_id: storyContainerId });
      } catch (storyErr) {
        console.warn(`[Scheduler] Multi-target Story share failed for post ${post.id} (account ${account.id}):`, storyErr.message);
        storyNotice = `Story share failed: ${storyErr.message}`;
      }
    }

    return { platformPostId: publishedId, notice: storyNotice };
  }

  if (account.platform === 'tiktok') {
    const { postToTikTok, refreshTikTokTokenForAccount } = require('./tiktok');
    let accessToken = account.access_token;
    if (account.token_expires_at && Date.now() > account.token_expires_at - 3600000 && account.refresh_token) {
      const refreshed = await refreshTikTokTokenForAccount(account.id, account.refresh_token);
      if (refreshed) accessToken = refreshed;
    }
    // Synthetic "user" object shaped exactly like what postToTikTok already
    // expects, so its proven upload/polling logic is reused as-is rather
    // than duplicated. `id` is deliberately the social_accounts row's own
    // id (not a real users.id) — if postToTikTok's own internal refresh
    // check also happens to fire, it updates zero rows instead of
    // overwriting a real user's default-account token.
    const fakeUser = {
      id: account.id,
      tiktok_access_token: accessToken,
      tiktok_refresh_token: account.refresh_token,
      tiktok_token_expires_at: account.token_expires_at,
    };
    const result = await postToTikTok(post, fakeUser);
    return { platformPostId: result.publishId || null };
  }

  if (account.platform === 'youtube') {
    const { postToYouTube, refreshYouTubeTokenForAccount } = require('./youtube');
    let accessToken = account.access_token;
    if (account.token_expires_at && Date.now() > account.token_expires_at - 300000 && account.refresh_token) {
      const refreshed = await refreshYouTubeTokenForAccount(account.id, account.refresh_token);
      if (refreshed) accessToken = refreshed;
    }
    const fakeUser = {
      id: account.id,
      ...
      youtube_token_expires_at: account.token_expires_at,
    };
    const result = await postToYouTube(post, fakeUser);
    return { platformPostId: result.videoId || null };
  }

  if (account.platform === 'linkedin') {
    const { postToLinkedIn, refreshLinkedInTokenForAccount } = require('./linkedin');
    let accessToken = account.access_token;
    if (account.token_expires_at && Date.now() > account.token_expires_at - 3600000 && account.refresh_token) {
      const refreshed = await refreshLinkedInTokenForAccount(account.id, account.refresh_token);
      if (refreshed) accessToken = refreshed;
    }
    const fakeUser = {
      id: account.id,
      linkedin_access_token: accessToken,
      linkedin_person_id: account.platform_account_id,
    };
    const result = await postToLinkedIn(post, fakeUser);
    return { platformPostId: result.postId || null };
  }

  // Multi-account version of the single-account "Path A" logic above (same
  // graph.instagram.com endpoints, same polling/story-share behavior) — that
  // path was built first and this one was missing entirely, which is why
  // every Instagram-direct account failed here with "not yet supported"
  // even though single-account publishing already worked fine.
  if (account.platform === 'instagram-direct') {
    const GRAPH_IG = 'https://graph.instagram.com/v23.0';
    const igUserId = account.platform_account_id;
    const igToken = token;
    // Missing on the first pass — alsoShareToStory was declared as a const
    // scoped only to the 'instagram' (Meta-linked) block above, so this
    // block threw "alsoShareToStory is not defined" the moment it got to
    // its own also-share-to-Story check. The actual Reel publish above that
    // check had already succeeded by then, which is why the content showed
    // up on Instagram correctly despite the scheduler logging a failure.
    const alsoShareToStory = !isStory && !!post.also_share_to_story;
    let creationId;
    ...
    if (creationId) {
      const pubResp = await fetch(`${GRAPH_IG}/${igUserId}/media_publish`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ access_token: igToken, creation_id: creationId }).toString(),
      });
      const pubJson = await pubResp.json();
      if (!pubResp.ok) throw new Error(pubJson.error?.message || `IG publish failed: HTTP ${pubResp.status}`);
      publishedId = pubJson.id || null;
    }

    // Same soft-failure treatment as every other platform's Story-share above.
    let storyNotice = null;
    if (alsoShareToStory && publishedId && post.media_url) {
      try {
        const storyParams = { access_token: igToken, media_type: 'STORIES' };
        ...
        const storyResp = await fetch(`${GRAPH_IG}/${igUserId}/media`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams(storyParams).toString(),
        });
        const storyJson = await storyResp.json();
        if (!storyResp.ok) throw new Error(storyJson.error?.message || 'Story container create failed');
        const storyContainerId = storyJson.id;
        ...
        if (post.media_type === 'video') {
          ...
          for (let i = 0; i < 40; i++) {
            await new Promise(r => setTimeout(r, 5000));
            const sParams = new URLSearchParams({ fields: 'status_code', access_token: igToken });
            const sResp = await fetch(`${GRAPH_IG}/${storyContainerId}?${sParams.toString()}`);
            const sJson = await sResp.json();
            if (sJson.status_code === 'FINISHED' || sJson.status_code === 'PUBLISHED') { storyReady = true; break; }
            if (sJson.status_code === 'ERROR' || sJson.status_code === 'EXPIRED') throw new Error('Story processing failed on Meta');
          }
          if (!storyReady) throw new Error('Story processing timed out after 200s');
        }
        ...
        const storyPubJson = await storyPubResp.json();
        if (!storyPubResp.ok) throw new Error(storyPubJson.error?.message || 'Story publish failed');
      } catch (storyErr) {
        console.warn(`[Scheduler] Multi-target (IG-direct) Story share failed for post ${post.id} (account ${account.id}):`, storyErr.message);
        storyNotice = `Story share failed: ${storyErr.message}`;
      }
    }
  ...
  throw new Error(`Multi-target publishing not yet supported for platform: ${account.platform}`);
}
...
function startScheduler() {
  ...
  cron.schedule('* * * * *', async () => {
    ...
    for (const post of due) {
      ...
      // ── Mass-posting path — explicit target accounts ────────────────────
      // Only present for posts created with accountIds[]. Every existing
      // post has zero rows here and falls straight through to the original
      // single-account flow below, completely unchanged.
      const targets = db.prepare(`
        SELECT pt.id as target_id, pt.status as target_status, sa.*
        FROM post_targets pt
        JOIN social_accounts sa ON sa.id = pt.social_account_id
        WHERE pt.post_id = ? AND pt.status != 'posted'
      `).all(post.id);
      ...
      if (targets.length > 0) {
        ...
        for (const target of targets) {
          try {
            const { platformPostId, notice } = await publishToTarget(post, target);
            db.prepare(`UPDATE post_targets SET status = 'posted', platform_post_id = ?, posted_at = ?, error_message = ? WHERE id = ?`)
              .run(platformPostId, now, notice || null, target.target_id);
            const label = `${target.platform}${target.account_name ? ' (' + target.account_name + ')' : ''}`;
            results.push(label);
            console.log(`[Scheduler] Multi-target publish success — ${label} for post ${post.id}${notice ? ` (note: ${notice})` : ''}`);
          } catch (e) {
            db.prepare(`UPDATE post_targets SET status = 'failed', error_message = ? WHERE id = ?`)
              .run(e.message, target.target_id);
            errors.push(`${target.account_name || target.platform}: ${e.message}`);
            console.error(`[Scheduler] Multi-target publish failed — ${target.platform} (${target.account_name}) for post ${post.id}:`, e.message);
          }
        }
        ...
        if (results.length > 0 && errors.length === 0) {
          db.prepare("UPDATE posts SET status = 'posted', posted_at = ?, error_message = NULL, retry_count = 0 WHERE id = ?")
            .run(now, post.id);
          await notifyUser('post-success', post.user_id, { caption: post.caption, platforms: results, scheduledTime: post.scheduled_time });
          ...
          await notifyUser('post-failed', post.user_id, { caption: post.caption, platforms: results, error: `Partial failure — ${errors.join('; ')}` });
        } else {
          const newRetryCount = (post.retry_count || 0) + 1;
          if (newRetryCount < MAX_RETRIES) {
            db.prepare("UPDATE posts SET status = 'scheduled', scheduled_time = ?, error_message = ?, retry_count = ? WHERE id = ?")
              .run(now + 5 * 60 * 1000, errors.join('; '), newRetryCount, post.id);
          } else {
            db.prepare("UPDATE posts SET status = 'failed', error_message = ?, retry_count = ? WHERE id = ?")
              .run(errors.join('; '), newRetryCount, post.id);
            await notifyUser('post-failed', post.user_id, { caption: post.caption, platforms: targets.map(t => t.platform), error: errors.join('; ') });
          }
        }
        continue; // this post is fully handled — skip the legacy single-account flow below
      }
