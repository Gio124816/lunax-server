...
// Covers Facebook Page + Instagram feed/Stories, plus TikTok and YouTube
// (reusing their existing single-account upload logic via a synthetic
// "user" object — see below). LinkedIn isn't included yet — no linkedin.js
// was available to wire up when this was written. Facebook Stories aren't
// covered either — Meta's Stories publishing for Pages works quite
// differently from the standard Graph API feed/photo/video endpoints used
// here, so it's left out rather than guessed at. Also still uses the
// simpler file_url video endpoint for Facebook rather than the full
// resumable-upload dance publishPost() uses, since that needs a separate
// user-level token alongside the page token that multi-account connections
// (possibly a different Meta login entirely) can't guarantee exists.
