...
async function publishPost(post, user) {
  ...
  // Only require Meta token if actually posting to Meta platforms
  const needsMeta = platforms.includes('Instagram') || platforms.includes('Facebook');
  if (needsMeta && !token) throw new Error('No Meta access token for user');
    ...
    try {
      if (post.media_url && post.media_type === 'video') {
        ...
        try {
          ...
          console.log(`[Scheduler] Facebook video publish success for post ${post.id}`);
        } catch (videoErr) {
