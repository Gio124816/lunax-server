...
// Deliberately narrower than publishPost() above: covers Facebook Page +
// Instagram business account feed posts (image/video) only — no Stories, no
// also_share_to_story, no TikTok/YouTube/LinkedIn yet. Also uses the
// simpler file_url video endpoint rather than the full resumable-upload
// dance publishPost() uses for Facebook video, because that flow needs a
// separate user-level token alongside the page token — accounts connected
// here (social_accounts) may belong to entirely different Meta logins (e.g.
// separate agency clients), so there's no guaranteed matching user token to
// pair with each page token the way a single owner's own account has.
