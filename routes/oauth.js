...
router.get('/meta/callback', async (req, res) => {
  ...
  try {
    ...
    let session;
    if(existingUserId) {
      // Link Meta to the existing logged-in account — save user token + page token
      db.prepare(`UPDATE users SET
        meta_access_token = ?,
        meta_page_id = COALESCE(?, meta_page_id),
        meta_page_name = COALESCE(?, meta_page_name),
        meta_page_token = COALESCE(?, meta_page_token),
        meta_ig_id = COALESCE(?, meta_ig_id),
        meta_ig_name = COALESCE(?, meta_ig_name),
        updated_at = ?
        WHERE id = ?`)
