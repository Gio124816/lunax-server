...
router.get('/google', (req, res) => {
  ...
  // Detect native app login — append :native to state so callback knows to redirect to lunax://
...
router.get('/google/callback', async (req, res) => {
  ...
  // Check if this is a native app login
  ...
  try {
    ...
    // Native app: redirect to lunax:// URL scheme so ASWebAuthenticationSession catches it
    if(isNative) {
    ...
    if(isNative) {
      return res.redirect(`lunax://auth?error=google_failed`);
    }
...
router.get('/meta', (req, res) => {
  ...
  const params = new URLSearchParams({
    ...
    scope: 'email,public_profile,instagram_basic,instagram_content_publish,instagram_manage_insights,pages_manage_posts,pages_read_engagement,pages_show_list,pages_manage_metadata,publish_video',
    state,
    response_type: 'code'
  });
...
router.get('/meta/callback', async (req, res) => {
  ...
  try {
    ...
    // Auto-fetch pages and save first page token so Facebook posting works immediately
    let autoPageId = null, autoPageToken = null, autoPageName = null;
    let autoIgId = null, autoIgName = null;
    try {
      const pagesRes = await fetch(`https://graph.facebook.com/v20.0/me/accounts?fields=id,name,access_token,instagram_business_account{id,name,username}&access_token=${longToken}`);
      ...
      if (pagesData.data && pagesData.data.length > 0) {
        ...
        if (firstPage.instagram_business_account) {
          ...
          autoIgName = firstPage.instagram_business_account.name || firstPage.instagram_business_account.username;
        }
      }
    } catch(e) {
    ...
    // Check if state is a JWT from an already logged-in user
    ...
    if(state) {
      try {
        ...
        // state is not a JWT — that's fine, fall through to normal flow
      }
    }

    let session;
    if(existingUserId) {
      // Link Meta to the existing logged-in account — always overwrite all token fields on reconnect
      ...
      // New user or unrecognized state — find or create
