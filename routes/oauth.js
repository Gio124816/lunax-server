...
function _upsertSocialAccount({ ownerId, platform, platformAccountId, accountName, accountHandle, accessToken, pageId, isDefault }) {
  ...
  if (existing) {
    db.prepare(`
      UPDATE social_accounts SET
        ...
        last_used_at = ?
      WHERE id = ?
    `).run(accountName || null, accountHandle || null, accessToken, pageId || null, now, existing.id);
    return existing.id;
  }
  const id = uuidv4();
  ...
  db.prepare(`
    INSERT INTO social_accounts (
      ...
      access_token, page_id, connected_at, last_used_at, is_default
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, ownerId, platform, platformAccountId, accountName || null, accountHandle || null,
    accessToken, pageId || null, now, now, (isDefault && !hasDefault) ? 1 : 0);
  return id;
}
