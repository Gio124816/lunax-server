...
router.post('/', requireAuth, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(`
    INSERT INTO posts (id, user_id, account_id, caption, hashtags, media_url, media_type, platforms, scheduled_time, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'scheduled')
  `).run(
    id, req.user.id, accountId || null, caption,
    JSON.stringify(hashtags || []), mediaUrl || null, mediaType || null,
