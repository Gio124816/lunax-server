...
router.post('/', requireAuth, (req, res) => {
  ...
  // Mass-posting: validate each requested account belongs to this owner,
  // then create one post_targets row per account. Silently skip any id
  // that doesn't resolve (wrong owner, typo, deleted account) rather than
  // failing the whole post — the rest of the valid targets still go out.
  if (Array.isArray(accountIds) && accountIds.length > 0) {
    const validAccounts = db.prepare(`
      SELECT id FROM social_accounts WHERE user_id = ? AND id IN (${accountIds.map(() => '?').join(',')})
...
router.get('/:id/targets', requireAuth, (req, res) => {
  const ownerId = _resolveOwnerId(req.user.id);
  const post = db.prepare('SELECT id FROM posts WHERE id = ? AND user_id = ?').get(req.params.id, ownerId);
  if (!post) return res.status(404).json({ error: 'Post not found' });
