...
// Kick: NOT IMPLEMENTED. streamkey:read is a real, documented OAuth scope
// (confirmed working — your consent screen showed it, and the connection
// itself succeeded), but I couldn't verify an actual REST endpoint for
// retrieving the stream key/URL from Kick's public API docs. One source
// building MCP tooling around Kick's API explicitly skipped this same
// capability for the same reason. Check docs.kick.com directly for the
// real endpoint before this can be wired up — guessing a URL here risks
// the same trial-and-error the redirect_uri and scope issues already cost.
async function getKickDestination(user) {
  if (!user.kick_access_token) return null;
  console.warn('[Restream] Kick stream-key fetch not implemented — endpoint unconfirmed, see comment above getKickDestination.');
  return null;
}
