...
router.get('/validate/:streamKey', (req, res) => {
  ...
  // Token column is 'refresh_token' in the social_accounts table.
  // Wrapped in try/catch so a schema mismatch never blocks stream validation.
  const platforms = [];

  const getStreamKeyForPlatform = (userId, platform) => {
    try {
      // Try refresh_token first (actual column name in DB), then access_token fallback
      const row = db.prepare(
        'SELECT refresh_token, access_token FROM social_accounts WHERE user_id = ? AND platform = ? LIMIT 1'
      ).get(userId, platform);
      return row?.refresh_token || row?.access_token || null;
    } catch (e) {
      console.warn(`[Validate] Could not query ${platform} stream key:`, e.message);
      return null;
    }
  };

  // Twitch
  const twitchToken = getStreamKeyForPlatform(user.id, 'twitch');
  if (twitchToken) {
    platforms.push({ name: 'Twitch', rtmpUrl: 'rtmp://live.twitch.tv/app', streamKey: twitchToken });
  }

  // YouTube Live
  const ytToken = getStreamKeyForPlatform(user.id, 'youtube-live') || getStreamKeyForPlatform(user.id, 'youtube');
  if (ytToken) {
    platforms.push({ name: 'YouTube', rtmpUrl: 'rtmp://a.rtmp.youtube.com/live2', streamKey: ytToken });
  }

  // Kick
  const kickToken = getStreamKeyForPlatform(user.id, 'kick');
  if (kickToken) {
    platforms.push({ name: 'Kick', rtmpUrl: 'rtmp://ingest.global-contribute.live-video.net/app', streamKey: kickToken });
  }
