...
router.get('/linkedin/callback', async (req, res) => {
  ...
  try {
    ...
    if (!decoded) return res.redirect('https://lunaxmedia.com?linkedin=error');
  ...
  try {
    ...
    // Same fix applied to Meta/TikTok/YouTube — attach to the shared owner,
    // not the individual team member's own row.
    ...
    db.prepare(`
      UPDATE users SET
        ...
        linkedin_avatar_url = ?
      WHERE id = ?
    `).run(access_token, refresh_token || null, expiresAt, personId, name, avatar, ownerId);

    // Multi-account: reconnecting with a different LinkedIn profile creates
    // a new row rather than overwriting, same as the other platforms.
    _upsertSocialAccount({
      ownerId, platform: 'linkedin',
...
// Per-account refresh for multi-account mode — writes to the specific
// social_accounts row instead of the singular users.linkedin_* columns.
// Note: LinkedIn only issues a usable refresh_token if your app has been
// approved for the relevant token-refresh product access; if it wasn't,
// refresh_token will be null and this simply won't have anything to work
// with — same limitation applies to the original single-account flow.
async function refreshLinkedInTokenForAccount(accountId, refreshToken) {
  try {
    ...
    const expiresAt = Date.now() + ((expires_in || 5183944) * 1000);
    db.prepare(`
    ...
    console.error('LinkedIn token refresh (multi-account) failed:', err.response?.data || err.message);
    return null;
  }
}
...
module.exports.refreshLinkedInTokenForAccount = refreshLinkedInTokenForAccount;
