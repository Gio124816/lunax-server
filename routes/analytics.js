...
async function fetchFbInsights(fbPostId, fbPageToken) {
  // Split into two independent requests — previously these were combined
  // into one `fields=` string, and Graph API rejects the WHOLE request if
  // even one requested field/metric is invalid. That's why likes/comments
  // (which were never the problem) were coming back as 0 too: a single bad
  // metric name inside insights.metric(...) was failing everything bundled
  // alongside it, not just impressions/reach.
  const baseFields = 'likes.summary(true),comments.summary(true),shares';
  const baseUrl = `${GRAPH_FB}/${fbPostId}?fields=${baseFields}&access_token=${fbPageToken}`;
  const baseResp = await fetch(baseUrl);
  const baseJson = await baseResp.json();
  if (!baseResp.ok || baseJson.error) {
    throw new Error(baseJson.error?.message || `FB base fields HTTP ${baseResp.status}`);
  }

  const result = {
    likes: baseJson.likes?.summary?.total_count || 0,
    comments: baseJson.comments?.summary?.total_count || 0,
    shares: baseJson.shares?.count || 0,
    reach: 0,
    impressions: 0,
    saves: 0,
    video_views: 0,
  ...
  // Insights metrics fetched separately and non-fatally — if Meta has
  // renamed/deprecated one of these (they do this periodically), this
  // should only zero out reach/impressions/clicks, never take down the
  // engagement numbers above that already succeeded.
  try {
    const metricsFields = 'insights.metric(post_impressions,post_reach,post_clicks)';
    const metricsUrl = `${GRAPH_FB}/${fbPostId}?fields=${metricsFields}&access_token=${fbPageToken}`;
    const metricsResp = await fetch(metricsUrl);
    const metricsJson = await metricsResp.json();
    if (!metricsResp.ok || metricsJson.error) {
      // Logged in full and unconditionally (not just a truncated message) so
      // the exact metric Meta is rejecting is visible in Railway logs on the
      // next real post — this API's valid metric names shift over time, and
      // guessing at a replacement name without seeing the real error is how
      // this got miscalled the first time.
      console.warn(`[Analytics] FB insights.metric() rejected for post ${fbPostId} — raw error:`, JSON.stringify(metricsJson.error || metricsJson));
    } else {
      const insights = {};
      if (metricsJson.insights && metricsJson.insights.data) {
        for (const metric of metricsJson.insights.data) {
          insights[metric.name] = metric.values?.[0]?.value || 0;
        }
      }
      result.reach = insights.post_reach || 0;
      result.impressions = insights.post_impressions || 0;
      result.clicks = insights.post_clicks || 0;
    }
  } catch (metricsErr) {
    console.warn(`[Analytics] FB insights.metric() fetch threw for post ${fbPostId}:`, metricsErr.message);
  }
