...
// ── DB MIGRATION ──────────────────────────────────────────────────────────────
// Ensure post_analytics table exists with all needed columns
try {
  db.prepare(`
...
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_analytics_user ON post_analytics(post_id)').run(); } catch (e) {}

// ── HELPERS ───────────────────────────────────────────────────────────────────

const GRAPH_IG = 'https://graph.instagram.com/v23.0';
const GRAPH_FB = 'https://graph.facebook.com/v23.0';
const CACHE_TTL = 60 * 60 * 1000; // 1 hour — don't hammer the API

// Fetch IG insights for a single media object via IG-direct token
async function fetchIgDirectInsights(igMediaId, igToken) {
  // graph.instagram.com insights endpoint
  // instagram_business_basic covers impressions, reach, likes_count, comments_count
  // For Reels: also plays, saved, shares
...
// Fetch FB post insights via page token
...
// Upsert analytics row for a post + platform
...
// Refresh analytics for a single post (called on-demand or by background job)
...
// Every hour, refresh analytics for all posted posts from the last 30 days
// that haven't been refreshed in the last hour.
async function runAnalyticsRefresh() {
  const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;
  const staleCutoff = Date.now() - CACHE_TTL;

  // Get all posted posts in last 30 days
  const posts = db.prepare(`
    ...
    WHERE p.status = 'posted'
      AND p.posted_at > ?
      AND (
        -- either no analytics row exists or it's stale
        NOT EXISTS (
...
// Start background job — stagger start so it doesn't run on boot
setTimeout(() => {
  runAnalyticsRefresh();
  setInterval(runAnalyticsRefresh, CACHE_TTL);
}, 30 * 1000); // wait 30s after server start

// ── ROUTES ────────────────────────────────────────────────────────────────────

// GET /analytics/summary
// Returns aggregate totals + top posts for the user's dashboard
router.get('/summary', requireAuth, (req, res) => {
  ...
  // Daily breakdown for chart (likes + reach per day)
...
// GET /analytics/posts
// Returns all posts with their analytics for the grid view
...
// POST /analytics/refresh/:postId
// Force-refresh analytics for a specific post
