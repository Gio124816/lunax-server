...
// Instagram's numeric IDs frequently exceed Number.MAX_SAFE_INTEGER (2^53-1,
// ~9 quadrillion) — these are 17-digit IDs. If a response sends one as a
// bare (unquoted) JSON number instead of a string, JSON.parse silently
// rounds it to the nearest representable float64 value, which for a number
// this large can genuinely change the last digit(s) — e.g. an ID ending in
// ...217 coming back out as ...216. That's not a hypothetical: it's exactly
// what happened here, traced by comparing the ID on the user's own
// Instagram authorization screen against what got stored and later sent to
// Meta at publish time. Reading the raw response text and quoting big bare
// integers BEFORE JSON.parse preserves the exact digit sequence Instagram
// actually sent, regardless of where in the pipeline it's read.
function parsePreservingBigInts(rawText) {
  const safe = rawText.replace(/:(\s*)(-?\d{16,})(\s*[,}])/g, ':$1"$2"$3');
  return JSON.parse(safe);
}

// ── STATE HELPERS — HMAC-signed to prevent CSRF (same pattern as tiktok.js/youtube.js/linkedin.js) ──
function createState(userId, isNative) {
  const payload = Buffer.from(JSON.stringify({ userId, ts: Date.now(), isNative: !!isNative })).toString('base64');
...
router.get('/instagram-direct', (req, res) => {
  ...
  const state = createState(userId, isNative);
...
router.get('/instagram-direct/callback', async (req, res) => {
  ...
  // Peek at isNative even before signature verification succeeds/fails, so
  // every error branch below can still route back to the native app instead
  // of stranding it on a plain webpage (same reasoning as oauth.js's meta
  // flow, which can tell isNative apart from the raw state string alone).
  let isNative = false;
  if (state) {
    try {
      const lastDot = state.lastIndexOf('.');
      const payload = lastDot === -1 ? state : state.slice(0, lastDot);
      const peeked = JSON.parse(Buffer.from(payload, 'base64').toString());
      isNative = !!peeked.isNative;
    } catch (e) { /* not parseable — fall through as web */ }
  }

  if (error) {
    console.error('Instagram-direct OAuth error:', error);
    if (isNative) return res.redirect('lunax://instagram-direct?error=instagram_denied');
    return res.redirect('https://lunaxmedia.com?instagram=error');
  }
  ...
  try {
    ...
    // transformResponse disables axios's normal JSON.parse so the raw text
    // can be parsed with parsePreservingBigInts instead — see that function
    ...
    const shortData = parsePreservingBigInts(shortRes.data);
    const { access_token: shortToken } = shortData;
    ...
    // Same raw-text + parsePreservingBigInts treatment — this response's
    // "id" is the one actually used for every publish call later
    // (/{id}/media, /{id}/media_publish in scheduler.js), so it's the
    // authoritative ID to store, not the token-exchange step's user_id.
    ...
    const profile = parsePreservingBigInts(profileRes.data) || {};
    ...
    if (isNative) {
      return res.redirect(`lunax://instagram-direct?name=${encodeURIComponent(name)}&connected=1`);
    }
    ...
    if (isNative) return res.redirect('lunax://instagram-direct?error=instagram_failed');
