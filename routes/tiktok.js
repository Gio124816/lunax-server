...
const CLIENT_KEY = isSandbox ? process.env.TIKTOK_SANDBOX_CLIENT_KEY : process.env.TIKTOK_CLIENT_KEY;
const CLIENT_SECRET = isSandbox ? process.env.TIKTOK_SANDBOX_CLIENT_SECRET : process.env.TIKTOK_CLIENT_SECRET;
console.log(`[TikTok] Mode: ${isSandbox ? 'SANDBOX' : 'PRODUCTION'} | Key: ${CLIENT_KEY?.slice(0,8)}...`);
...
router.get('/tiktok/callback', async (req, res) => {
  ...
  try {
    ...
    if (!decoded) return res.redirect('https://lunaxmedia.com?tiktok=error');
  ...
  try {
    ...
    // Same fix applied to Meta's flow earlier — a team member connecting
    // TikTok should attach it to the shared owner account, not their own
    // otherwise-invisible row.
    ...
    // Multi-account: reconnecting with a different TikTok account creates a
    // new row (matched on platform_account_id) rather than overwriting —
    // this is what lets more than one TikTok account stay connected at once.
    _upsertSocialAccount({
      ownerId, platform: 'tiktok',
      platformAccountId: open_id || ttUser.open_id,
      accountName: ttUser.display_name || 'TikTok Account',
...
// Same as refreshTikTokToken above, but for a specific connected account in
// multi-account mode — writes to that account's own social_accounts row
// instead of the singular users.tiktok_* columns, so refreshing a
// non-default account's token can't corrupt the default account's token.
async function refreshTikTokTokenForAccount(accountId, refreshToken) {
  try {
    const res = await axios.post('https://open.tiktokapis.com/v2/oauth/token/', {
      client_key: CLIENT_KEY,
      client_secret: CLIENT_SECRET,
    ...
    `).run(access_token, refresh_token || refreshToken, expiresAt, accountId);
    return access_token;
  } catch (err) {
    console.error('TikTok token refresh (multi-account) failed:', err.response?.data || err.message);
    return null;
  }
}
...
module.exports.refreshTikTokTokenForAccount = refreshTikTokTokenForAccount;
