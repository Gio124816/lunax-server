...
// After fetching ads, we enrich any video creatives with:
//   - resolvedThumb: full-res poster frame from GET /{video_id}?fields=picture
//   - resolvedVideoUrl: playable MP4 from GET /{video_id}?fields=source
// This is done in parallel so it adds minimal latency. Image ads use
// image_url (full-res) from the creative directly — no extra fetch needed.
router.get('/ads/creatives', requireAuth, async (req, res) => {
  try {
    ...
    // Fetch ads with full creative subfields. Note: video_url is NOT a valid
    // creative field in Meta API v20+ — video data is accessed via video_id.
    const fields = 'id,name,status,creative{id,name,thumbnail_url,image_url,video_id,object_story_spec{page_id,video_data{video_id,image_url,message,title,call_to_action},link_data{picture,message,name,description,link,call_to_action}}},insights{spend,reach,impressions,clicks,ctr,cpc,actions}';
    const data = await metaGet(`/${accountId}/ads`, token, {
      fields, limit: 30,
      ...(since && { time_range: JSON.stringify({ since, until }) }),
    });

    // Enrich video ads: fetch full-res poster + playable source in parallel
    const ads = data.data || [];
    await Promise.allSettled(ads.map(async (ad) => {
      // Resolve video_id from multiple possible locations in the creative tree
      ...
      if (videoId) {
        try {
          ...
          // picture = full-res poster frame (replaces blurry thumbnail_url)
          // source  = direct MP4 URL for in-app playback
          ad.creative = ad.creative || {};
          ad.creative.resolvedThumb    = videoData.picture || null;
          ad.creative.resolvedVideoUrl = videoData.source  || null;
          ad.creative.resolvedVideoId  = videoId;
        } catch (e) {
          // Non-fatal — fall back to thumbnail_url on the frontend
          console.warn(`[creatives] could not enrich video ${videoId}:`, e.message);
        }
      } else {
        // Image ad — image_url is already full-res, nothing extra needed
        ad.creative = ad.creative || {};
        ad.creative.resolvedThumb = ad.creative.image_url
