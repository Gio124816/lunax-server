    ...
    if (pageId && pageToken) {
      try {
        ...
        (feed.data || []).forEach(post => {
          // Resolve a thumbnail for whatever this post is (photo, video, link) so
          // the inbox can show what the comment is actually about.
          ...
          const postMedia = mediaUrl ? { url: mediaUrl, permalink: post.permalink_url || null, type: mediaType } : null;
    ...
    if (igId) {
      try {
        ...
        mediaWithComments.forEach(result => {
          ...
          const postMedia = postMediaUrl ? { url: postMediaUrl, permalink: post.permalink || null, type: isVideo ? 'video' : 'image' } : null;
