...
router.get('/ads/creatives', requireAuth, async (req, res) => {
  try {
    ...
    if (!token) return res.status(400).json({ error: 'No Meta token' });
    const { since, until } = req.query;
    const fields = 'id,name,status,ad_id,creative{id,name,thumbnail_url,image_url,video_url,object_story_spec{video_data{video_id,image_url},link_data{picture,message,name,call_to_action}}},insights{spend,reach,impressions,clicks,ctr,cpc,actions}';
