...
// ── Auto-migration: ensure the users table has a column to store the selected
// Meta ad account. Safe to run on every boot — if the column already exists,
// SQLite throws a "duplicate column" error which we swallow. This avoids having
// to edit the central schema file.
try {
  db.prepare('ALTER TABLE users ADD COLUMN meta_ad_account_id TEXT').run();
  console.log('[meta] added users.meta_ad_account_id column');
} catch (e) {
  // Column already exists (duplicate column name) — expected on subsequent boots
  if (!/duplicate column/i.test(e.message)) {
    console.warn('[meta] meta_ad_account_id migration note:', e.message);
  }
}
