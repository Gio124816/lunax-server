...
// Self-contained migration — table for "hide from Luna X" on DM messages,
// which Meta's Send APIs don't let us actually delete/unsend on their side.
try {
  db.prepare(`
    CREATE TABLE IF NOT EXISTS inbox_hidden_messages (
      id TEXT PRIMARY KEY,
  ...
  console.error('[Inbox] Failed to ensure inbox_hidden_messages table:', e.message);
}
...
// Resolves the effective "owner" id — a team member's own row has no Meta
// token/pages of its own; they act on behalf of whoever they're a team
// member of. See routes/team.js for the full model.
...
// Enriches every "us" message across all threads with the real team member
// who sent it, via Luna X's own attribution log. Meta's API has no concept
// of "which of our users sent this" — every reply from every team member
// looks identical to Meta, coming from the same shared Page token — so this
// is the only place that distinction can come from.
function _attachInboxAttribution(threads, ownerId) {
  const ids = [];
  threads.forEach(t => (t.messages || []).forEach(m => { if (m.from === 'us') ids.push(m.id); }));
  if (!ids.length) return;
  const placeholders = ids.map(() => '?').join(',');
  const rows = db.prepare(`
    SELECT aa.target_id, u.name as senderName
    FROM action_attributions aa
    JOIN users u ON u.id = aa.acted_by_id
    WHERE aa.team_owner_id = ? AND aa.action_type = 'comment_reply' AND aa.target_id IN (${placeholders})
  `).all(ownerId, ...ids);
  const byTarget = {};
  rows.forEach(r => { byTarget[r.target_id] = r.senderName; });
  threads.forEach(t => (t.messages || []).forEach(m => {
    if (m.from === 'us' && byTarget[m.id]) m.senderName = byTarget[m.id];
  }));
}

// DMs (Messenger/IG) have no delete/unsend endpoint on Meta's side, so "delete"
// for a message can only ever mean "stop showing it in Luna X" — this filters
// those hidden ids back out on every fetch so the removal persists.
function _filterHiddenMessages(threads, ownerId) {
  const rows = db.prepare(`SELECT message_id FROM inbox_hidden_messages WHERE team_owner_id = ?`).all(ownerId);
  if (!rows.length) return;
  const hidden = new Set(rows.map(r => r.message_id));
  threads.forEach(t => {
    if (t.messages) t.messages = t.messages.filter(m => !hidden.has(m.id));
  });
}
...
async function metaDelete(path, token, params = {}) {
  const url = new URL(`${API_BASE}${path}`);
  url.searchParams.set('access_token', token);
  Object.entries(params).forEach(([k, v]) => url.searchParams.set(k, v));
...
// The actual, literal use of the pages_manage_metadata permission — tells
// Meta to start pushing this Page's feed/comment activity to our webhook
// (routes/webhook.js) instead of only ever being polled for it. Deliberately
// its own explicit endpoint (not silently auto-called on every connect) so
// there's a concrete, visible action to demonstrate in the App Review
// screencast: connect the Page, then trigger this, then show a real comment
// arriving as a webhook event afterward — all tied to the same Page.
router.post('/subscribe-page', requireAuth, async (req, res) => {
  try {
    const user = db.prepare('SELECT meta_page_id, meta_page_token FROM users WHERE id = ?').get(req.user.id);
    if (!user?.meta_page_id || !user?.meta_page_token) {
      return res.status(400).json({ error: 'No connected Facebook Page to subscribe' });
    }

    // "feed" covers new posts and comments on posts — the same content
    // pages_read_user_content already reads via polling and
    // pages_manage_engagement already replies to; this just adds the
    // real-time push path on top of the same underlying data.
    const params = new URLSearchParams({
      subscribed_fields: 'feed',
      access_token: user.meta_page_token,
    });
    const r = await fetch(`https://graph.facebook.com/v23.0/${user.meta_page_id}/subscribed_apps?${params.toString()}`, {
      method: 'POST',
    });
    ...
    console.log(`[Meta] Page ${user.meta_page_id} subscribed to webhooks — success: ${json.success}`);
    res.json({ success: true, pageId: user.meta_page_id });
  } catch (e) {
    handleMetaError(e, res);
  }
});
...
router.post('/post', requireAuth, async (req, res) => {
  try {
    const { igId, pageId, caption, captionFacebook, hashtags, mediaUrl, mediaType, scheduledTime, platforms, postId } = req.body;
    const token = getUserMetaToken(req.user.id);
    if (!token) return res.status(400).json({ error: 'No Meta token' });
    ...
    if ((platforms || []).includes('Instagram') && igId) {
      ...
      if (creationId) {
        // media_publish's response id is the actual published media's id —
        // this IS the ig_post_id analytics needs, not creationId itself.
        const published = await metaPost(`/${igId}/media_publish`, token, { creation_id: creationId });
    ...
    if ((platforms || []).includes('Facebook') && pageId) {
      let fbResult = null;
      if (mediaUrl && mediaType === 'video') {
        fbResult = await metaPost(`/${pageId}/videos`, pageToken, {
          file_url: mediaUrl, description: fullCaption,
          ...(isFuture && { scheduled_publish_time: unixTime }),
        });
      } else if (mediaUrl && mediaType === 'image') {
        fbResult = await metaPost(`/${pageId}/photos`, pageToken, {
          url: mediaUrl, caption: fullCaption,
          ...(isFuture && { scheduled_publish_time: unixTime, published: false }),
        });
      } else {
        fbResult = await metaPost(`/${pageId}/feed`, pageToken, {
          message: fullCaption,
          ...(isFuture && { scheduled_publish_time: unixTime, published: false }),
        });
      }
      // Photos/videos return both `id` (the photo/video object) and `post_id`
      // (the actual Page post) — post_id is what FB's insights endpoint
      // expects; feed posts only ever return `id`, which IS the post id.
      fbPostId = fbResult?.post_id || fbResult?.id || null;
      results.push('Facebook');
    }

    // This is the write-back that was missing entirely: without it, status
    // never left 'scheduled', posted_at was never stamped, and ig_post_id/
    // fb_post_id were never saved — so Posted Content and Analytics had
    // nothing to show no matter how much actually went out to Meta.
    // Only stamps posted_at for genuinely-immediate publishes; a future
    // scheduled_publish_time means Meta hasn't actually posted it yet.
    if (postId && !isFuture && (igPostId || fbPostId)) {
      try {
        db.prepare(`
          UPDATE posts SET
            status = 'posted',
            ...
            updated_at = ?
          WHERE id = ?
        `).run(Date.now(), igPostId, fbPostId, Date.now(), postId);
      } catch (dbErr) {
        // Non-fatal — the post already succeeded on Meta's side either way.
        console.error('[Post] Failed to write back posted status:', dbErr.message);
      }
    }

    res.json({ success: true, posted: results, igPostId, fbPostId });
  } catch (e) {
    handleMetaError(e, res);
  }
});
...
router.get('/inbox', requireAuth, async (req, res) => {
  try {
    ...
    const socialAccts = db.prepare("SELECT * FROM social_accounts WHERE user_id = ?").all(req.user.id);
    console.log('[Inbox] social_accounts:', JSON.stringify(socialAccts).slice(0, 300));

    const pageId = user?.meta_page_id;
    const igId   = user?.meta_ig_id;

    // Page token lives on the users row (meta_page_token).
    // social_accounts stores oauth tokens, not page tokens.
    let pageToken = user?.meta_page_token || null;
    ...
    if (pageId && pageToken) {
      try {
        ...
        (fbConvs.data || []).forEach(conv => {
          ...
          // Get name from participants or from the first message's 'from' field
          const other = (conv.participants?.data || []).find(p => String(p.id) !== String(pageId));
          // Try to get name from messages (more reliable than participants in v20)
          const firstTheirMsg = msgs.find(m => m.from?.id && String(m.from.id) !== String(pageId));
          const senderName = firstTheirMsg?.from?.name || other?.name || 'Messenger';
          threads.push({
            id: 'fb-' + conv.id,
            platform: 'fb',
            type: 'dm',
            name: senderName,
            initials: senderName.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase() || 'F',
            avatarBg: '#1877f2',
            ...
            messages: msgs.map(m => ({
              id: m.id,
              from: m.from?.id && String(m.from.id) === String(pageId) ? 'us' : 'them',
              senderName: m.from?.name || '',
              text: m.message || '',
              time: relTime(m.created_time),
              editable: false // Messenger's Send API has no edit/unsend endpoint
            }))
          });
        });
      } catch (e) {
    ...
    if (pageId && pageToken) {
      try {
        const feed = await metaGet(`/${pageId}/feed`, pageToken, {
          fields: 'id,message,created_time,full_picture,permalink_url,attachments{media_type,url,media},comments{id,message,from,created_time,comments{id,message,from,created_time}}',
          limit: 10
        });
        console.log('[Inbox] FB feed posts:', (feed.data||[]).length, '| first post comments:', JSON.stringify((feed.data||[])[0]?.comments).slice(0,200));
        // Resolve post media (with an async pass for video source URLs, since
        // that requires a follow-up call per video — the feed response only
        // gives a thumbnail, not a playable source)
        await Promise.all((feed.data || []).map(async post => {
          let mediaUrl = post.full_picture || null;
          let mediaType = 'image';
          let videoSrc = null;
          const att = (post.attachments?.data || [])[0];
          if (att && att.media_type === 'video') {
            mediaType = 'video';
            mediaUrl = att.media?.image?.src || post.full_picture || null;
            const videoId = att.target?.id;
            if (videoId) {
              try {
                const videoData = await metaGet(`/${videoId}`, pageToken, { fields: 'source' });
                videoSrc = videoData.source || null;
              } catch (e) {
                // Non-fatal — falls back to poster image + "open original" link
              }
            }
          }
          const postMedia = mediaUrl ? { url: mediaUrl, permalink: post.permalink_url || null, type: mediaType, videoSrc } : null;
          ...
          (post.comments?.data || []).forEach(comment => {
            ...
            threads.push({
              ...
              type: 'comment',
              name: commenterName,
              initials: commenterName.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase() || 'F',
              avatarBg: '#1877f2',
              ...
              media: postMedia,
              messages: [
                { id: comment.id, from: 'them', text: comment.message || '', time: relTime(comment.created_time), editable: false },
                ...replies
              ]
            });
          });
        ...
        (igConvs.data || []).forEach(conv => {
          ...
          threads.push({
            ...
            messages: msgs.map(m => ({
              ...
              text: m.message || '',
              time: relTime(m.created_time),
              editable: false // Instagram messaging has no edit/unsend endpoint either
            }))
          });
        });
      } catch (e) {
    ...
    if (igId) {
      try {
        // First fetch media IDs, then fetch comments on each
        const igMedia = await metaGet(`/${igId}/media`, token, {
          fields: 'id,caption,timestamp,media_type,media_url,thumbnail_url,permalink',
          limit: 15
        });
        console.log('[Inbox] IG media posts:', (igMedia.data||[]).length);

        // Fetch comments for each post in parallel (IG doesn't support nested comments in media list)
        const mediaWithComments = await Promise.allSettled(
          (igMedia.data || []).slice(0, 10).map(async post => {
            try {
        ...
        mediaWithComments.forEach(result => {
          if (result.status !== 'fulfilled') return;
          const post = result.value;
          // Reels/videos don't reliably return a directly-playable media_url via
          // this field set, but thumbnail_url is always a static poster image —
          // good enough to show "what this comment is on" at a glance.
          const isVideo = post.media_type === 'VIDEO' || post.media_type === 'REELS';
          const postMediaUrl = isVideo ? (post.thumbnail_url || post.media_url) : post.media_url;
          // media_url IS the playable source for IG video/reels (unlike FB,
          // which only gives a thumbnail from the feed and needs a follow-up
          // call) — so we can embed it directly, no extra request needed.
          const postMedia = postMediaUrl
            ? { url: postMediaUrl, permalink: post.permalink || null, type: isVideo ? 'video' : 'image', videoSrc: isVideo ? (post.media_url || null) : null }
            : null;
          ...
          (post.comments?.data || []).forEach(comment => {
            const replies = (comment.replies?.data || []).map(r => ({
              ...
              editable: false // Instagram's API doesn't support editing comment text
            }));
            const igUser = comment.username || 'Instagram User';
            threads.push({
              id: 'ig-comment-' + comment.id,
              platform: 'ig',
              type: 'comment',
              name: '@' + igUser,
              initials: igUser.slice(0, 2).toUpperCase(),
              avatarBg: '#e1306c',
              time: relTime(comment.timestamp),
              unread: replies.length === 0, // unread if no replies yet
              done: false,
              rawId: comment.id,
              postId: post.id,
              igId,
              media: postMedia,
              messages: [
                { id: comment.id, from: 'them', senderName: '@' + igUser, text: comment.text || '', time: relTime(comment.timestamp), editable: false },
                ...replies
              ]
            });
          });
        ...
        console.log('[Inbox] IG comments found:', threads.filter(t=>t.platform==='ig').length);
      } catch (e) {
        console.warn('[Inbox] IG Comments error:', e.message);
    ...
    _attachInboxAttribution(threads, _resolveOwnerId(req.user.id));
    _filterHiddenMessages(threads, _resolveOwnerId(req.user.id));
...
router.post('/inbox/reply', requireAuth, async (req, res) => {
  try {
    ...
    // Attribution — records which real team member actually sent this reply.
    // Meta's Send/Comments APIs return different id fields depending on the
    // endpoint (message_id for DMs, id for comment replies) — either one is
    // the canonical id that will reappear as this message's own `id` next
    // time GET /meta/inbox rebuilds the thread, so it's the right join key.
    // Non-fatal: the reply already succeeded on Meta's side either way.
    try {
      const replyId = result?.message_id || result?.id;
      if (replyId) {
        ...
        `).run(uuidv4(), ownerId, req.user.id, replyId, Date.now());
      }
    } catch (attrErr) {
      console.error('[Inbox] Failed to log reply attribution:', attrErr.message);
    }
...
// ── PATCH /meta/inbox/comment ──
// Edit the text of a comment WE made (a reply). Facebook's API allows a Page
// to edit comments it authored; Instagram's Graph API has no equivalent —
// there is no endpoint to update comment text, only to delete or hide one.
router.patch('/inbox/comment', requireAuth, async (req, res) => {
  try {
    const token = getUserMetaToken(req.user.id);
    if (!token) return res.status(400).json({ error: 'No Meta token' });
    const { platform, rawId, message, pageToken } = req.body;
    if (!message?.trim()) return res.status(400).json({ error: 'Message required' });

    if (platform === 'ig') {
      return res.status(400).json({
        error: 'Instagram doesn\'t support editing comment text — you can delete and re-reply instead.',
        code: 'UNSUPPORTED_BY_PLATFORM'
      });
    }
    if (platform !== 'fb') {
      return res.status(400).json({ error: 'Unsupported platform for comment edit' });
    }

    const tok = pageToken || token;
    const result = await metaPost(`/${rawId}`, tok, { message: message.trim() });
    ...
    try {
      ...
      db.prepare(`
        INSERT INTO action_attributions (id, team_owner_id, acted_by_id, action_type, target_id, created_at)
        VALUES (?, ?, ?, 'comment_edit', ?, ?)
      `).run(uuidv4(), ownerId, req.user.id, rawId, Date.now());
    } catch (attrErr) {
      console.error('[Inbox] Failed to log edit attribution:', attrErr.message);
    }
...
// ── DELETE /meta/inbox/comment ──
// Deletes a comment on Facebook or Instagram. Page/business owners can delete
// ANY comment on their own post's media (moderation), not just their own replies.
router.delete('/inbox/comment', requireAuth, async (req, res) => {
  try {
    const token = getUserMetaToken(req.user.id);
    if (!token) return res.status(400).json({ error: 'No Meta token' });
    const { platform, rawId, pageToken } = req.body;
    if (!rawId) return res.status(400).json({ error: 'rawId required' });

    const tok = platform === 'fb' ? (pageToken || token) : token;
    await metaDelete(`/${rawId}`, tok);

    try {
      const ownerId = _resolveOwnerId(req.user.id);
      db.prepare(`
        INSERT INTO action_attributions (id, team_owner_id, acted_by_id, action_type, target_id, created_at)
        VALUES (?, ?, ?, 'comment_delete', ?, ?)
      `).run(uuidv4(), ownerId, req.user.id, rawId, Date.now());
    } catch (attrErr) {
      console.error('[Inbox] Failed to log delete attribution:', attrErr.message);
    }
...
// ── POST /meta/inbox/hide-message ──
// Messenger and Instagram DMs have no delete/unsend endpoint in the Graph API,
// so this can only remove a message from Luna X's own view — it stays on the
// actual platform. Persisted per-team so it survives refreshes and reloads.
router.post('/inbox/hide-message', requireAuth, async (req, res) => {
  try {
    const { messageId } = req.body;
    if (!messageId) return res.status(400).json({ error: 'messageId required' });
    const ownerId = _resolveOwnerId(req.user.id);
    db.prepare(`
      INSERT INTO inbox_hidden_messages (id, team_owner_id, message_id, created_at)
      VALUES (?, ?, ?, ?)
    `).run(uuidv4(), ownerId, messageId, Date.now());
    res.json({ success: true });
  } catch (e) {
