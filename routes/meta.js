...
router.get('/ads/creatives', requireAuth, async (req, res) => {
  try {
    ...
    if (!token) return res.status(400).json({ error: 'No Meta token' });
    const { since, until } = req.query;
    // video_url is NOT a valid field on creative objects — Meta removed it.
    // Use video_id and fetch the source separately via GET /{video_id}?fields=source
    // (the /meta/ads/:adId/video endpoint handles this for the frontend).
