...
router.get('/youtube/callback', async (req, res) => {
  ...
  try {
    ...
    if (!decoded) return res.redirect('https://lunaxmedia.com?youtube=error');
  ...
  try {
    ...
    // Same fix as Meta's and TikTok's flows — attach to the shared owner,
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(access_token, refresh_token || null, expiresAt, channelId, channelName, channelAvatar, ownerId);

    // Multi-account: reconnecting with a different Google/YouTube channel
    // creates a new row rather than overwriting, same as TikTok/Meta.
    _upsertSocialAccount({
      ownerId, platform: 'youtube',
...
// Same as refreshYouTubeToken above, but scoped to a specific connected
// account's social_accounts row for multi-account mode.
async function refreshYouTubeTokenForAccount(accountId, refreshToken) {
  try {
    const res = await axios.post('https://oauth2.googleapis.com/token', {
      client_id: CLIENT_ID,
      client_secret: CLIENT_SECRET,
    ...
    console.error('YouTube token refresh (multi-account) failed:', err.response?.data || err.message);
    return null;
  }
}
...
module.exports.refreshYouTubeTokenForAccount = refreshYouTubeTokenForAccount;
