...
async function downloadVod(importId, userId, url, platform) {
  ...
  try {
    ...
    // Wrapped on its own — a failure here (e.g. a FOREIGN KEY edge case on
    // user_id) shouldn't undo an already-successful download + S3 upload.
    // Nothing in the Video Editor flow actually depends on postId existing.
    let postId = null;
    try {
      postId = uuidv4();
      db.prepare(`
      ...
      console.error(`[VOD] Import ${importId}: draft post record failed, import still succeeded — ${e.message}`);
      postId = null;
    }
...
function getVideoInfo(url) {
  return new Promise((resolve, reject) => {
    const args = ['--dump-json', '--no-playlist', '--quiet',
