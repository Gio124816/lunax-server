...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    if (posts.length === 0) {
      return res.json({ comments: [], note: 'No published posts found with external IDs yet.' });
    }

    // Fetch comments for each post in parallel (cap at 10 posts to avoid rate limits)
    const allComments = [];
    const fetchJobs = posts.slice(0, 10).map(async (post) => {
      ...
      try {
        ...
        const items = (json.data || []).map(c => ({
          ...
          type: 'comment',
          senderName: ctx.platform === 'facebook'
        ...
        console.error(`[Inbox] Failed to fetch comments for post ${post.id}:`, e.message);
      }
    });
