...
// so the Inbox can show "Replied by [name]" — this is Luna X's own internal
// attribution, since Meta's Graph API has no concept of sub-identity behind
// a single shared Page token.
function _attachReplyAttribution(items) {
  if (!items.length) return;
  const ids = items.map(i => i.id);
  const placeholders = ids.map(() => '?').join(',');
  const rows = db.prepare(`
    SELECT aa.target_id, u.name as repliedByName
  ...
  `).all(...ids);
  const byTarget = {};
  rows.forEach(r => { byTarget[r.target_id] = r.repliedByName; });
  items.forEach(i => { if (byTarget[i.id]) i.repliedByName = byTarget[i.id]; });
}
...
router.post('/:commentId/reply', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      `).run(uuidv4(), ownerId, req.user.id, req.params.commentId, Date.now());
    } catch (attrErr) {
