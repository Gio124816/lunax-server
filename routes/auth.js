...
//
// REQUIRED env var: APPLE_CLIENT_ID — a comma-separated list of every
// bundle ID that calls this endpoint, one per platform, e.g.
// "com.lunaxmedia.lunax,com.lunaxmedia.LunaX-iOS" on Railway. Native Sign in
// with Apple (as opposed to the web/JS flow) puts the calling app's own
// Bundle ID in the token's `aud` claim, and macOS/iOS are separate apps with
// separate bundle IDs — a single value here would only ever match one
// platform's token and silently break Sign In with Apple on the other.
//
  ...
  return new Promise((resolve, reject) => {
    jwt.verify(
      ...
      getApplePublicKey,
      {
        ...
        // macOS and iOS are two separate apps with two different bundle IDs
        // (com.lunaxmedia.lunax vs com.lunaxmedia.LunaX-iOS), and each one's
        // Sign In with Apple token carries its OWN bundle ID in `aud` — a
        // single audience value can only ever match one platform's token.
        // APPLE_CLIENT_ID is now a comma-separated list so both are
        // accepted here, e.g. "com.lunaxmedia.lunax,com.lunaxmedia.LunaX-iOS".
...
router.get('/restream/oauth/:platform', (req, res) => {
  console.log('[RESTREAM] Authorize route hit — platform:', req.params.platform, 'hasToken:', !!req.query.token);
...
router.get('/restream/oauth/:platform/callback', async (req, res) => {
  console.log('[RESTREAM] Callback route hit — platform:', req.params.platform, 'query keys:', Object.keys(req.query));
  const { platform } = req.params;
  const config = RESTREAM_PLATFORMS[platform];
  ...
  try {
    ...
    if (error) {
      console.log('[RESTREAM] Provider returned error — platform:', platform, 'error:', error, 'description:', error_description || '(none)');
      return fail(error);
    }
    ...
    try {
      ...
      if (platform === 'twitch') {
        displayName = profileJson?.data?.[0]?.display_name || '';
        // Needed as broadcaster_id for GET /helix/streams/key later — the
        // stream-key API requires this specific numeric ID, not just the
        // access token, and nothing else in this flow captures it.
        twitchUserId = profileJson?.data?.[0]?.id || null;
      }
      else if (platform === 'youtube') displayName = profileJson?.items?.[0]?.snippet?.title || '';
      else if (platform === 'kick') {
        // Confirmed shape: { data: [{ user_id, name, email, profile_picture }], message: "OK" }
    ...
    // expires_in is seconds-from-now per each provider's token response —
    // stored as an absolute timestamp so restream logic can check "is this
    // token still good" without redoing that math every time.
    ...
    if (platform === 'twitch' && twitchUserId) {
      db.prepare(`UPDATE users SET twitch_user_id = ?, twitch_token_expires_at = ? WHERE id = ?`)
        .run(twitchUserId, expiresAt, userId);
    } else if (platform === 'youtube') {
      db.prepare(`UPDATE users SET youtube_restream_token_expires_at = ? WHERE id = ?`)
        .run(expiresAt, userId);
    } else if (platform === 'kick') {
      db.prepare(`UPDATE users SET kick_token_expires_at = ? WHERE id = ?`)
        .run(expiresAt, userId);
    }
