...
router.post('/apple', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // beta-onboarding logic. password_hash stays NULL: this user only
      // ever signs in via Apple, never via /login's email+password path.
      const userId = uuidv4();
      const now = Date.now();
      ...
      db.prepare(`
        ...
        VALUES (?, ?, NULL, ?, '', ?, 1, ?, ?)
      `).run(userId, emailToStore, 'Apple User', appleUserId, now, now);
