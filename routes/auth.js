...
router.post('/request-access', async (req, res) => {
  const { name, business, instagram } = req.body;
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO access_requests (name, business, instagram) VALUES (?, ?, ?)`
    ).run(name, business, instagram || '');
    ...
    try {
      ...
      await sgMail.send({
        to: 'giovanni@arderemedia.com',
        from: 'noreply@lunaxmedia.com',
        subject: `Luna X — New Access Request from ${business}`,
        html: `
          <h2 style="font-family:sans-serif">New Luna X Access Request</h2>
          <p style="font-family:sans-serif"><strong>Name:</strong> ${name}</p>
          <p style="font-family:sans-serif"><strong>Business:</strong> ${business}</p>
      ...
      console.error('Request access email failed:', emailErr.message);
