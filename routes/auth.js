  ...
  try {
    const user = db.prepare(`
      SELECT id, email, name, business_name, created_at,
             ...
             youtube_channel_id, youtube_channel_name, youtube_channel_avatar,
...
router.post('/request-access', async (req, res) => {
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO access_requests (name, email, instagram) VALUES (?, ?, ?)`
    ).run(name, email, instagram || '');
    ...
    try {
      ...
      await sgMail.send({
        ...
        subject: `Luna X — New Access Request from ${name}`,
        html: `
