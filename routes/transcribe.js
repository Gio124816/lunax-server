...
// AssemblyAI v3 API
const ASSEMBLYAI_API = 'https://api.assemblyai.com';
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const submitRes = await fetch(`${ASSEMBLYAI_API}/v3/transcripts`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        audio_url: videoUrl,
    ...
    while (Date.now() < deadline) {
      ...
      const pollRes = await fetch(`${ASSEMBLYAI_API}/v3/transcripts/${transcriptId}`, { headers });
