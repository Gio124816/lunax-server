...
const PLANS = {
  starter: {
    name: 'Starter',
    priceId: process.env.STRIPE_PRICE_STARTER,       // e.g. $29/mo
    price: 29,
  ...
  pro: {
    name: 'Pro',
    priceId: process.env.STRIPE_PRICE_PRO,           // e.g. $79/mo
    price: 79,
  ...
  agency: {
    name: 'Agency',
    priceId: process.env.STRIPE_PRICE_AGENCY,        // e.g. $199/mo
    price: 199,
...
router.get('/subscription', requireAuth, (req, res) => {
  ...
  res.json({
    subscription: {
      ...
      isTrialActive,
      trialDaysLeft,
      // Beta users are always active with full pro access
...
router.post('/checkout', requireAuth, async (req, res) => {
  try {
    ...
    if (!plan.priceId) return res.status(400).json({ error: 'Plan not configured — contact support' });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        trial_period_days: sub?.status === 'trial' ? Math.max(0, Math.ceil((sub.trial_ends_at - Date.now()) / 86400000)) : 0,
    ...
    if (!sub?.stripe_customer_id) {
      if (sub) {
        db.prepare('UPDATE subscriptions SET stripe_customer_id = ? WHERE user_id = ?').run(stripeCustomerId, req.userId);
      } else {
        db.prepare('INSERT INTO subscriptions (user_id, plan, status, stripe_customer_id, created_at) VALUES (?, ?, ?, ?, ?)')
          .run(req.userId, planKey, 'pending', stripeCustomerId, Date.now());
      }
    }
...
// In server.js, add: app.use('/billing/webhook', express.raw({ type: 'application/json' }))
        ...
        if (user && event.type === 'customer.subscription.created') {
          await sendEmail({
            ...
            html: `
              ...
              <a href="${process.env.FRONTEND_URL}" style="display:inline-block;padding:12px 24px;background:#7c6dfa;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Open Luna X →</a>
            `
          });
        }
        break;
...
function requireActiveSubscription(req, res, next) {
  ...
  // Beta users always have full pro access — never blocked
  if (sub.is_beta) {
    req.subscription = sub;
    req.plan = PLANS.pro; // beta = full pro
    req.isBeta = true;
    return next();
  }
