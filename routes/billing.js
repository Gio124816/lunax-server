...
router.get('/beta-status', (req, res) => {
  ...
  const betaActive = process.env.BETA_MODE !== 'ended';
  res.json({
    ...
    message: betaActive
      ...
      : remaining > 0
        ? `${remaining} free beta spots remaining`
        : 'Beta is full – start a 14-day free trial'
  });
});

// — END BETA (admin only) ————————————————————
// POST /billing/end-beta with header x-admin-secret
// Converts all beta/trial users to 14-day trials so they get prompted to subscribe
router.post('/end-beta', (req, res) => {
  ...
  // Give all beta users a 14-day trial window before requiring payment
  const result = db.prepare(`
    UPDATE subscriptions
    SET status = 'trial', trial_ends_at = ?, is_beta = 0, updated_at = ?
    WHERE is_beta = 1
  `).run(trialEnd, now);

  // Also reset any still-in-trial users who haven't added a card yet
  db.prepare(`
    UPDATE subscriptions
    SET trial_ends_at = ?, updated_at = ?
    WHERE status = 'trial' AND trial_ends_at < ?
  `).run(trialEnd, now, now + 24 * 60 * 60 * 1000); // only extend if expiring within 24hrs

  console.log(`[Billing] Beta ended — ${result.changes} beta users converted to 14-day trial`);
  res.json({ success: true, converted: result.changes });
});
...
const PLANS = {
  ...
  pro: {
    ...
    features: [
      'Unlimited posts',
      '5 social accounts',
      'AI captions + video transcription',
      'Bulk scheduler',
      'Video editor',
      'Meta Ads dashboard',
...
function requireActiveSubscription(req, res, next) {
  // During beta — everyone gets full Pro access
  if (process.env.BETA_MODE !== 'ended') {
    req.subscription = { plan: 'pro', status: 'beta', is_beta: true };
    req.plan = PLANS.pro;
    req.isBeta = true;
    return next();
  }
  ...
  // Beta users (is_beta flag still set) get Pro access
  if (sub.is_beta) {
  ...
  if (!isActive) {
    return res.status(402).json({
      ...
      status: sub.status,
      trialEnded: sub.status === 'trial' && sub.trial_ends_at <= Date.now()
    });
  }
